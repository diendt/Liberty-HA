...
scp -r ./04-1-add-glance.sh root@$CON_MGNT_IP2:/root/script/04-1-add-glance.sh

ssh root@$CON_MGNT_IP2  bash -ex << EOF
	cd script
	source config.cfg
	source function.sh
	/root/script/04-1-add-glance.sh $VIP $CON_MGNT_IP2
EOF
...
scp -r ./04-1-add-glance.sh root@$CON_MGNT_IP3:/root/script/04-1-add-glance.sh

ssh root@$CON_MGNT_IP3  bash -ex << EOF
    cd script
	source config.cfg
	source function.sh
	/root/script/04-1-add-glance.sh $VIP $CON_MGNT_IP3
EOF
