...
./03-1-keystone-add.sh $VIP $CON_MGNT_IP1
...
scp -r ./03-1-keystone-add.sh root@$CON_MGNT_IP2:/root/script/03-1-keystone-add.sh

ssh root@$CON_MGNT_IP2  bash -ex << EOF
	cd script
	source config.cfg
	source function.sh
	/root/script/03-1-keystone-add.sh $VIP $CON_MGNT_IP2
	#/root/script/03-1-keystone-add.sh
EOF

echocolor "Install Openstack components on $CON_MGNT_IP3"
sleep 3

scp -r ./03-1-keystone-add.sh root@$CON_MGNT_IP3:/root/script/03-1-keystone-add.sh

ssh root@$CON_MGNT_IP3  bash -ex<< EOF
	cd script
	source config.cfg
	source function.sh
	/root/script/03-1-keystone-add.sh $VIP $CON_MGNT_IP3
	#/root/script/03-1-keystone-add.sh
EOF
