...
sleep 3
scp ./02-1-ntp.sh root@$CON_MGNT_IP2:/root/script/02-1-ntp.sh
ssh root@$CON_MGNT_IP2 '/root/script/02-1-ntp.sh'

echocolor "INSTALL NTP NODE $CON_MGNT_IP3"
sleep 3
scp ./02-1-ntp.sh root@$CON_MGNT_IP3:/root/script/02-1-ntp.sh
ssh root@$CON_MGNT_IP3 '/root/script/02-1-ntp.sh'
...
scp -r ./02-2-mariadb.sh root@$CON_MGNT_IP2:/root/script/02-2-mariadb.sh

ssh root@$CON_MGNT_IP2  << EOF
    cd script
	source config.cfg
	source function.sh
	/root/script/02-2-mariadb.sh $CON_MGNT_IP2
EOF
...
scp -r ./02-2-mariadb.sh root@$CON_MGNT_IP3:/root/script/02-2-mariadb.sh

ssh root@$CON_MGNT_IP3  << EOF
    cd script
	source config.cfg
	source function.sh
	/root/script/02-2-mariadb.sh $CON_MGNT_IP3
EOF
