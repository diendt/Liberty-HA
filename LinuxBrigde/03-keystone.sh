...
scp -r ./03-1-keystone-add.sh root@$CON_MGNT_IP2:/root/ha-lb/03-1-keystone-add.sh

ssh root@$CON_MGNT_IP2  bash -ex << EOF
	cd ha-lb
	source config.cfg
	source function.sh
	#/root/ha-lb/03-1-keystone-add.sh $VIP $CON_MGNT_IP2
	/root/ha-lb/03-1-keystone-add.sh
EOF

echocolor "Install Openstack components on $CON_MGNT_IP3"
sleep 3

scp -r ./03-1-keystone-add.sh root@$CON_MGNT_IP3:/root/ha-lb/03-1-keystone-add.sh

ssh root@$CON_MGNT_IP3  bash -ex<< EOF
	cd ha-lb
	source config.cfg
	source function.sh
	#/root/ha-lb/03-1-keystone-add.sh $VIP $CON_MGNT_IP3
	/root/ha-lb/03-1-keystone-add.sh
EOF
