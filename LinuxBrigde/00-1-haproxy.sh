...
ssh-copy-id root@$HA_IP1
ssh-copy-id root@$HA_IP2
cat << EOF > ~/.ssh/config
...
scp /etc/haproxy/haproxy.cfg root@$HA_IP2:/etc/haproxy/
scp /etc/keepalived/keepalived.conf root@$HA_IP2:/etc/keepalived/

ssh -t root@$HA_IP2 bash << EOF
sed -i 's/MASTER/BACKUP/' /etc/keepalived/keepalived.conf
sed -i 's/200/150/' /etc/keepalived/keepalived.conf
service keepalived restart
service harproxy
EOF
