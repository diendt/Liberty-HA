...
scp -r ./06-1-add-neutron.sh ./06-2-l3-agent-neutron.sh root@$CON_MGNT_IP2:/root/ha-lb/        
#scp -r ./06-1-add-neutron.sh root@$CON_MGNT_IP2:/root/ha-lb/06-1-add-neutron.sh

ssh root@$CON_MGNT_IP2  bash -ex << EOF
	cd ha-lb
	source config.cfg
	source function.sh
	/root/ha-lb/06-1-add-neutron.sh $VIP $CON_MGNT_IP2
	/root/ha-lb/06-2-l3-agent-neutron.sh
	reboot
EOF
...
#Install Neutron On Node 3
scp -r ./06-1-add-neutron.sh ./06-2-l3-agent-neutron.sh root@$CON_MGNT_IP3:/root/ha-lb/

ssh root@$CON_MGNT_IP3 bash -ex << EOF
	cd ha-lb
	source config.cfg
	source function.sh
	/root/ha-lb/06-1-add-neutron.sh $VIP $CON_MGNT_IP3
	/root/ha-lb/06-2-l3-agent-neutron.sh
	reboot
EOF
